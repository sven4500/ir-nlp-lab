...
std::vector<unsigned int> getDocID(std::ifstream& fin, unsigned int const termID)
{
    ...
    {
        // Извлекаем первый байт последовательности. И читаем сколько всего
        // байт было отведено для хранения этой дельты (последние два бита
        // самого старшего байта который хранится первым).
